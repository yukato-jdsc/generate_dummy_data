...
- マート名: `m_商品_all`
- CSVファイル名： `m_商品_all.csv`, `m_商品_all_diff.csv`
- 全量更新 or 差分更新: 全量更新
- 更新頻度: 日次
- データ数: 全件（122,802件）
- データ量: 219.76MB
...
| MVNO識別ID | `mvno_identification_id` | DECIMAL | 3,0 | － | - |
...
| チャージ額 | `charge_amount` | DECIMAL | 6,0 | － | - |
| ユニバーサル使用料 | `uni_universal_usage_fee` | DECIMAL | 4,0 | － | - |
| 利用有効期間 | `validity_period` | DECIMAL | 5,0 | － | - |
...
| 標準入数 | `standard_quantity` | DECIMAL | 10,3 | － | - |
| 出荷時入数 | `shipping_quantity` | DECIMAL | 6,0 | － | - |
| 個装箱サイズ_縦(mm) | `individual_box_size_length` | DECIMAL | 4,0 | － | - |
| 個装箱サイズ_横(mm) | `individual_box_size_width` | DECIMAL | 4,0 | － | - |
| 個装箱サイズ_高さ(mm) | `individual_box_size_height` | DECIMAL | 4,0 | － | - |
| 梱包財_紙重量(g) | `paper_packaging_material_weight` | DECIMAL | 7,0 | － | - |
| 梱包財_プラ重量(g) | `plastic_packaging_material_weight` | DECIMAL | 7,0 | － | - |
| 商品重量(g) | `product_weight` | DECIMAL | 7,0 | － | - |
| パレット積み付け数 | `number_of_pallets` | DECIMAL | 8,0 | － | - |
| 梱包仕様等 | `packaging_specifications_etc` | DECIMAL | 4,0 | － | - |
| MRP管理者コード | `mrp_administrator_code` | VARCHAR | 6 | － | - |
| 変換代表フラグ | `conversion_representative_flag` | VARCHAR | 2 | － | - |
| 検品用商品タイプコード | `product_type_for_inspection_cd` | VARCHAR | 6 | － | - |
| 検品用商品タイプ正式名称 | `product_type_for_inspection_nm` | VARCHAR | 1200 | － | - |
| MODEL_ID | `model_id` | DECIMAL | 10,0 | － | - |
| ISMIタイプ | `imsi_type` | VARCHAR | 4 | － | - |
| ISMIタイプ名称 | `imsi_type_official_name` | VARCHAR | 1200 | － | - |
